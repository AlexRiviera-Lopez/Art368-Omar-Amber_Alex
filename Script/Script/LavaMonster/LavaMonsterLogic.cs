    ...
    void Update()
    {
	    ...
	    if(HasFightStarted)
		{
            Countdown -= Time.deltaTime;
		      if(Countdown<= 0f){
				  Pillar1.transform.position = new Vector3(DelayedPlayerPosition.x,transform.position.y + -6f,DelayedPlayerPosition.z);
...
//lava moster to die when 4 trigger are pressed
//laava monster to move pillars around the game based on player location 2 seconds ago
